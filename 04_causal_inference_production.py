...
def load_state_level_data():
    ...
    state_data = (
        ...
        .agg(
            {
                "Plant annual net generation (MWh)": lambda x: pd.to_numeric(
                    x, errors="coerce"
                ).sum(),
                "Plant annual CO2 emissions (tons)": lambda x: pd.to_numeric(
                    x, errors="coerce"
                ).sum(),
            }
        )
...
def run_difference_in_differences(data, treated_states, treatment_year):
    ...
    return {
        ...
        "event_study": {"time": event_time, "coefficients": coefficients},
    }
...
def visualize_results(
    data, did_results, sc_results, treatment_year, treated_states, plot: bool = False
):
    ...
    if plot:
        ...
        ax1.plot(
            treated_trend.index[post_mask],
            treated_trend.values[post_mask],
            "o-",
            linewidth=2,
            markersize=6,
            color="#e74c3c",
        ...
        ax1.plot(
            control_trend.index[post_mask],
            control_trend.values[post_mask],
            "s-",
            linewidth=2,
            markersize=6,
            color="#3498db",
